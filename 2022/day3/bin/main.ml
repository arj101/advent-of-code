...
let group_common_item r1 r2 r3 =
  ...
  Seq.filter (fun i1 -> String.contains r2 i1 && String.contains r3 i1) r1_seq
  |> List.of_seq
...
let part2 () =
  ...
  let rec find_priority_sum channel accum_sum =
    try
      ...
      find_priority_sum channel
        (accum_sum + group_common_item_priority_sum line1 line2 line3)
    with End_of_file -> accum_sum
  in
  let priority_sum = find_priority_sum ic 0 in

...
let print_vis_map map =
  ...
  let rec print_row y =
    let rec print_tree x sub_row =
      if x == 15 && y == 50 then print_string "\x1b[32;1m";
      ...
      (if sub_row == 0 then (
         ...
         print_string char1;
         print_string char2);
      if x == 15 && y == 50 then print_string "\x1b[0m";
...
let look_from_left_right trees vis_map =
  ...
  let rec aux loc_accum y =
    ...
    in
    if y < h - 1 then aux loc_accum (y + 1) else loc_accum
  in
  aux [] 0
...
let tree_vis_map tree =
  ...
  let visible_locs =
    look_from_left_right tree vis_map @ look_from_top_bottom tree vis_map
  in
  (vis_map, visible_locs)

let part1 () =
  let ic = open_in "./input.txt" in
  let tree = parse ic in
  let vis_map, visible_locs = tree_vis_map tree in
  print_vis_map vis_map;
  Printf.printf "%d trees are visible\n" (visible_count vis_map);
  (tree, vis_map, visible_locs)

(* dist_[dir] functions do not check for boundaries because they are only run on trees that are not visible from boundaries *)
let dist_right tree_map (x, y) =
  let tree_height = Array2.get tree_map x y in
  let rec walk xoff =
    if Array2.get tree_map (x + xoff) y >= tree_height then xoff
    else walk (xoff + 1)
  in
  walk 1

let dist_left tree_map (x, y) =
  let tree_height = Array2.get tree_map x y in
  let rec walk xoff =
    if Array2.get tree_map (x + xoff) y >= tree_height then -xoff
    else walk (xoff - 1)
...
let dist_top tree_map (x, y) =
  let tree_height = Array2.get tree_map x y in
  let rec walk yoff =
    if Array2.get tree_map x (y + yoff) >= tree_height then -yoff
    else walk (yoff - 1)
  in
  walk (-1)

let dist_btm tree_map (x, y) =
  let tree_height = Array2.get tree_map x y in
  let rec walk yoff =
    if Array2.get tree_map x (y + yoff) >= tree_height then yoff
    else walk (yoff + 1)
...
let part2 tree_map vis_map visible_locs =
  let w, h = trees_dimension tree_map in
  let highest_scenic_score =
    List.fold_left
      (fun highest_score (x, y) ->
        if x <= 0 || y <= 0 || x >= w - 1 || y >= h - 1 then highest_score
        else
          let visibility = Array2.get vis_map x y in

          let y1 =
            if Vis.top_is_visible visibility then y else dist_top tree_map (x, y)
          and y2 =
            if Vis.bottom_is_visible visibility then h - 1 - y
            else dist_btm tree_map (x, y)
          and x1 =
            if Vis.left_is_visible visibility then x
            else dist_left tree_map (x, y)
          and x2 =
            if Vis.right_is_visible visibility then w - 1 - x
            else dist_right tree_map (x, y)
          in

          let scenic_score = y1 * y2 * x1 * x2 in
          if highest_score < scenic_score then scenic_score else highest_score)
      0 visible_locs
  in
  Printf.printf "Highest scenic score = %d\n" highest_scenic_score

let () =
  print_endline "-- part1 --";
  let tree_map, vis_map, visible_locs = part1 () in
  print_endline "\n-- part2 --";
  part2 tree_map vis_map visible_locs
